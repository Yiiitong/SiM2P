...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        print('UnetModel: tab_mode:', tab_mode, 'tab_dim:', tab_dim)
        ...
        if tab_dim is not None:
            self.tab_embed = nn.Sequential(
                linear(tab_dim, time_embed_dim),
                nn.SiLU(),
                linear(time_embed_dim, time_embed_dim),
            )
    ...
    def forward(self, x, timesteps, xT=None, y=None, **model_kwargs):
        """
        ...
        if self.tab_dim is not None:
            tabular_data = model_kwargs.get('tabular', None)
            assert tabular_data is not None, "Tabular data must be provided."
            tab = self.tab_embed(tabular_data)
            if self.tab_mode == 'add':
                emb = emb + tab
            elif self.tab_mode == 'concat':
                emb = th.cat([emb, tab], dim=1)
            else:
                raise ValueError(f"Invalid tab_mode: {self.tab_mode}")
