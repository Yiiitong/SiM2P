...
from .dit3d import DiT, DiT3D_models
import numpy as np

NUM_CLASSES = 4

def get_workdir(exp):
    workdir = f'SiM2P_workdir/{exp}'
    return workdir
...
def create_model(
    ...
    print(f"====== model backbone type: {unet_type} =========")
    if unet_type == 'adm':
        return UNetModel(
            dims=dims,
            image_size=image_size,
        ...
        print(f"using DiT type: {dit_type}")
        return DiT3D_models[dit_type](
            image_size=image_size,
            in_channels=in_channels,
            tab_dim=tab_dim,
            tab_mode=tab_mode,
        ...
        raise ValueError(f"Unsupported backbone type: {unet_type}")
